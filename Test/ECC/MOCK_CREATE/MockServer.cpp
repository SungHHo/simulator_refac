...
void SendSystemStatus(SOCKET client, const std::string& iniPath) {
    ...
    radar.position = { ReadLongLong("MFR", "posX", iniPath), ReadLongLong("MFR", "posY", iniPath) };
    ...
    ls.position = { ReadLongLong("LS", "posX", iniPath), ReadLongLong("LS", "posY", iniPath) };
    ls.mode = (uint8_t)ReadInt("LS", "mode", iniPath);
    ls.angle = ReadDouble("LS", "launchAngle", iniPath);
 
    LCStatus lc;
    lc.id = ReadInt("LC", "commandReady", iniPath);
    lc.position = { ReadLongLong("LC", "posX", iniPath), ReadLongLong("LC", "posY", iniPath) };
