...
StepMotorController::StepMotorController()
{
    ...
    Logger::log("[StepMotorController] Initializing with device: " + config.device + ", BaudRate: " + std::to_string(config.uartBaudRate));

    device = config.device;
    uartBaudRate = config.uartBaudRate;
...
void StepMotorController::sendCommand(const std::string &cmd)
{
    if (uart_fd < 0)
    {
        Logger::log("[StepMotorController] UART not initialized");
        return;
    }
