...
{
    long long x, y;
    statManager.positionBriefing(x, y);
    MissileInfo missile{x, y, command.altitude, command.speed, command.launch_angle};
    std::vector<uint8_t> packet = missile.toBytes();
    std::cout << "[Missile Info Packet Created]\n";
    std::cout << "  Position: (" << x << ", " << y << ")\n";
    std::cout << "  Altitude: " << command.altitude << "\n";
    std::cout << "  Speed: " << command.speed << "\n";
    std::cout << "  Launch Angle: " << command.launch_angle << "\n";
